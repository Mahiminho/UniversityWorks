...
int Generator::ConvertToPostfixForm(int i)
{
	vector <int> Stack;
	int n = 0;
	for (; ((Data->LexTable[i + n].type != LSeparator) && (Data->LexTable[i + n].type != LGoto)); ++n);	//Встановлення коректності та довжини вхідного масиву
	int k = i + n;
	for (; i <= k; i++)
	{
		TypeOfLex in;
...
void Generator::PrintCode(FILE* f)
{
	...
	for (;; ++i)
	{
		...
		if (l.type == LIf)
		{
			...
			for (; Data->LexTable[i].type != LEOF; i++)
			{
