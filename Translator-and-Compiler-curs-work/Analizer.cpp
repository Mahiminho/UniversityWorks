#include "Analizer.h"
#define AorN(ch) ((isalpha(ch) || isdigit(ch)) || ch == '#')
string backup;
int line = 1, bln = 1;
...
int Analizer::AnalisisLexems(FILE* FIn)
{
	...
	do
	{
		...
		//printf("Name: %10s\n", Data->LexTable[destIterator].name.c_str());
		destIterator++;
		if (backup.size() == 0)
		{
			bln = line;
	...
	for (size_t i = 0; i < Data->LexTable.size(); i++)
	{
		...
		if (Data->LexTable[i].type == LIdentifier)
		{
			...
			if (Data->LexTable[i].name.size() <= Data->TName.size())
			{
				...
				{
					if (islower(Data->LexTable[i].name[0]))
					{
						Data->LexTable[i].type = LUnknown;
						break;
					}
					...
					if (isupper(Data->LexTable[i].name[j]))
					{
						Data->LexTable[i].type = LUnknown;
						break;
					}
				...
				if (Data->LexTable[i + 1].type == LDDot)
				{
					Data->LexTable[i].name += Data->LexTable[i + 1].name;
					Data->LexTable[i].type = LLabel;
					Data->LexTable.erase(Data->LexTable.begin() + i + 1);
				}
			}
			else
	...
	for (size_t i = 0; i + 2 < Data->LexTable.size(); i++)
	{
		...
		if (Data->LexTable[i].type == LGoto && Data->LexTable[i + 1].type == LIdentifier)
		{
			Data->LexTable[i + 1].type = LTarget;
		}
...
void Analizer::PrintLexemsInFile()
{
	...
	for (size_t i = 0; i < Data->LexTable.size(); ++i)
	{
		...
		case LGoto: type = "goto"; break;
		case LLabel: type = "label"; break;
		case LTarget: type = "target"; break;
