...
int ErrorsCheck::IsExpression(int i, FILE* ef)
{
	...
	for (; (Data->LexTable[nom].type != LSeparator) && (Data->LexTable[nom].type != LStart) && (Data->LexTable[nom].type != LGoto); nom++)
	{
		if (Data->LexTable[nom].type == LRBraket)
		{
			if (!(IsOperation(Data->LexTable[nom + 1].type)) && (Data->LexTable[nom + 1].type != LSeparator) && (Data->LexTable[nom + 1].type != LRBraket) && (Data->LexTable[nom + 1].type != LStart) && (Data->LexTable[nom + 1].type != LGoto))
			{
...
int ErrorsCheck::ErrorChecking()
{
	...
	if (Data->EnableProgN)
	{
		...
		//{
		//	Err_num++;
		//	fprintf(ef, "%d - line %d: \tProgramName must be app!\n", Err_num, Data->LexTable[i].line);
		//}
	}
	if (!Data->PosLStart)
	{
	...
	for (j = 0;; j++)
	{
		...
		if (Data->LexTable[j].type == LIf)
		{
			...
			{
				if (Data->LexTable[i].type == LTarget)
				{
					if (Data->LexTable[i + 1].type != LSeparator)
					{
						Err_num++;
						fprintf(ef, "line %d: \t';' expected!\n", Data->LexTable[j + 1].line);
						break;
				...
				else if (Data->LexTable[i].type == LIf || Data->LexTable[i].type == LSeparator || Data->LexTable[i].type == LEnd)
				{
					Err_num++;
					fprintf(ef, "line %d: \t'%s' expected after '%s'!\n", Data->LexTable[j + 1].line, Data->LexName[LGoto].name.c_str(), Data->LexName[LIf].name.c_str());
					break;
				}
			}
			buf = IsExpression((j + 1), ef);
		...
		if (Data->LexTable[j].type == LTarget)
		{
			...
			for (size_t i = 0; i < Data->LexTable.size(); i++)
			{
				if (Data->LexTable[i].name == (Data->LexTable[j].name + ":"))
				{
					enable = 1;
					break;
				}
			}
			!enable ? fprintf(ef, "line %d: \tUnknown target!(%s)\n", Data->LexTable[j].line, Data->LexTable[j].name.c_str()), Err_num += 1 : NULL;
		}
